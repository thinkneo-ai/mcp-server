...
The server is production-deployed as a single Docker container, serving MCP on `/mcp` and the A2A Agent Card at `/.well-known/agent.json`. All 72 MCP tools and 24 A2A skills run in the same process, share the same PostgreSQL database, and are wrapped with the same free-tier middleware (500 calls/month, auto-provisioned API keys).

---

## MCP Tools

72 tools across 12 categories. All tools accept JSON-RPC 2.0 via Streamable HTTP.
...
No credit card required. All 72 tools available on the free tier.
